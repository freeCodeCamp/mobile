...
class SymbolBarService {
  ...
  Future<void> _loadState() async {
    try {
      ...
      print('Error loading symbol bar state: $e. Resetting to default.');
      _state = SymbolBarState.defaultState();
      await _saveState();
    }
  }
  ...
  Future<void> _saveState() async {
    try {
      await _prefs.setString(_activeSetKey, jsonEncode(_state.activeSet.toJson()));
      ...
      print('Error saving symbol bar state: $e');
    }
  }
  ...
  Future<void> updateCustomSet({
    ...
    final index = _state.customSymbolSets.indexWhere((set) => set.name == currentName);
    if (index == -1) {
  ...
  Future<void> deleteCustomSet(String setName) async {
    final index = _state.customSymbolSets.indexWhere((set) => set.name == setName);
    if (index == -1) {
