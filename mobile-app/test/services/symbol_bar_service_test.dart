...
import 'symbol_bar_service_test.mocks.dart';
...
void main() {
  group('SymbolSetModel Tests', () {
    test('SymbolSetType.fromValue should parse string correctly', () {
      ...
      expect(SymbolSetType.fromValue('invalid'),
          SymbolSetType.python); // Default fallback
    });
    ...
    test('SymbolBarState.toJson and fromJson should round-trip', () {
      final customSet = CustomSymbolSet(name: 'Test Set', symbols: ['a', 'b']);
  ...
  group('SymbolBarService Tests', () {
    ...
    test('getAllSymbolSets should return all sets', () async {
      ...
      expect(
        allSets.whereType<PredefinedSymbolSet>().length,
        3,
      );
      expect(
        allSets.whereType<CustomSymbolSet>().length,
        1,
      );
    });
